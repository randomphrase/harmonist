...
def test_reconcile_records_transitions_to_activity(tmp_path):
    ...
    for each real transition (→ Needs Linking, → Library), and a closing summary.
    ...
    assert any("Needs Linking (reconciled" in m for m in msgs)
...
def test_reconcile_reports_live_inbox_library_counts(tmp_path):
    ...
    all orphans) + the running tallies: → Library, → Needs Linking, → stuck New."""
    music = tmp_path / "music"
