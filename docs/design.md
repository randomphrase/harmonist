...
So both places that judge a disk title against MusicBrainz's accept it: `compare.album_fields` for the album panel, and `gardener._countable` for the update flag and its verdict. The accepted spelling is built by `models.title_with_disambiguation` from the release already in hand, which costs no extra MusicBrainz request — `disambiguation` is a core field on the release entity.
