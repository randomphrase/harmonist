...
the inbox with the side-by-side and Confirm suggestion on it. Worth
...
4. **If `©cmt` mentions any `bandcamp.com` URL AND MB has at least one Bandcamp URL relationship for the release:** write a sidecar with `store_url` set to MB's canonical Bandcamp URL, `bandcamp.item_id=None` (filled in later by sync). The album shows as **Needs Linking** until the next sync resolves the item_id.
...
**Needs Linking** (not Complete), and the next sync fills in `item_id`. When the
...
3. Purchases the title couldn't pin to an album → record them as an **ambiguous
   ...
   with no single `item_id`. The album leaves Needs Linking for **Complete** — it's
...
waiting to link (**Needs Linking**) whose purchase is *old* would never be seen. So
if any album is in Needs Linking at sync start, Harmonist **clears the checkpoint**
...
resolves every Needs Linking album — it either links it, or surrenders it.
...
After the backfill and the post-sync mis-tag pass, an album still in Needs Linking
...
The inbox also surfaces a Needs Linking album with two manual affordances:
**Wrong MusicBrainz match**, beside the release badge (reuse rematch → Needs MBID,
preserving store/purchase information and on-disk tags), and **Mark purchased
elsewhere**, beside the Bandcamp badge (clear `store_url`, drop the bandcamp block →
Complete).
...
- **Surrender = Needs MBID with a read-only suggestion.** When a full sync finds
  ...
  Needs Linking) with a "no purchase found" note and the seed/fix tools.
...
**A length decides confidence here, and nothing on the album page** (#550). The two are separate questions and the answers deliberately diverge: ranking a candidate and downgrading a match are what lengths are *for*, while the album page's findings are things a re-tag settles — and a length is not one, because nothing writes a duration into a file. A confirmed album whose files are seconds longer than MusicBrainz says therefore raises no finding, tints no note and offers no **Apply updates**; its tracklist states MusicBrainz's lengths in a muted column of their own and explains, on that column's heading, that the remedies are a different release or an edit on MusicBrainz. Counting them as findings put eight rows of an album behind a warning whose only button moved none of them — the one finding on the page that no action on it could ever clear.

Mechanically this is `compare.FieldComparison.comparable` doing what it has always meant: *a field Harmonist writes from MusicBrainz*. Such a field is never a finding whatever its agreement, and `Length` is the field that makes the rule bite. `FieldComparison.advisory` is the other half — a real disagreement, stated and never marked — and it is what earns the MusicBrainz length column and keeps the value off the row's MusicBrainz line, which lists pending changes only.
...
- **a fresh listing retires the previous candidate bytes** before measuring or fetching its front cover (#510). Only a 304 validates keeping them, even if the image URL is unchanged. A definitive absence also retires the candidate; a failed or deliberately skipped download leaves no old image masquerading as the new answer. A later 304 can retry a missing candidate when its stored width beats the album's current best. This affects only disposable archive copies, never images in the library;
...
**The flag accepts it; the panel does not** (#547). `gardener._countable` skips it, so the album is never an update to take and never reaches the Inbox. `compare.album_fields` reports it like any other difference, because the panel's question is not "do my files match MusicBrainz" but "do my files match what Harmonist would WRITE from this release" (`tagger.tagsets_for`) — and under a transform, or without one, one of the two spellings loses. The accepted spelling is built by `models.title_with_disambiguation` from the release already in hand, which costs no extra MusicBrainz request — `disambiguation` is a core field on the release entity.

The panel accepted it too until #547, and what that cost is the reason the two parted company: `compare.advisory` is built from the panel's rows, and `_album_update.html` gates the whole finding block on it — so silencing the row took the page's **Apply updates** button with it. A user who had turned the transform on could see neither the change nor a control that would apply it. Hiding a difference and declining to flag it are not the same act, and only the second one was wanted.
...
- **A second release country is not a difference** (#346). Picard writes whichever country `preferred_release_countries` matches (`picard/mbjson.py`, `release_to_metadata`), so a library tagged that way carries a code that is not MusicBrainz's first and is not stale either. Both places that judge a disk country accept any country **this release** names — `compare.album_fields` for the panel and `gardener._countable` for the flag — with the limit that it is the release's own release events, never "any country". A code MusicBrainz does not list for the release is genuinely stale and still reported.

**The panel keeps this one, where the album title lost it** (#547), and the asymmetry is the point rather than an oversight. A disambiguated title is *resolvable*: tick the transform and the two spellings agree, so reporting it offers the reader something to do. A second release country is not — Harmonist has no `preferred_release_countries` setting, so a Country row drawn as a difference is one the reader can only silence by giving in to MusicBrainz's answer, permanently, on every Picard-tagged multi-country album. A difference worth showing is one that can be acted on. If a country transform is ever added, this row moves with it.

**Only the flag and the panel are tolerant** — the same sentence as the album title's. A re-tag that happens for another reason puts MusicBrainz's `country` back on the file, and says so in the history.
...
- **Turning one on does not flood the Inbox.** The flag asks whether the disk value is in the accepted set, so a file holding the other spelling offers no update: nothing lands in the Inbox and #32's first night is uneventful. Without the invariant this would be #283's failure mode with the sign flipped — a whole library reported as needing an update over a setting nobody meant as one.
...
**Where the tolerance lives, and why not in the diff (#545).** Every rule above filters at the **flag**, never out of `tagger._changes_for`. That diff is three readers at once — the write-skip, the update flag, and the audit record the undo is built from — and `formats.write_tags` writes the whole `TagSet` rather than the diff, so deleting an entry from it does not stop the value changing on disk. It only stops the history saying so, which is exactly what it did: a re-tag that happened for some other reason replaced the user's album title and left no way back.
...
**No retained backup means no replacement** (#470). A backup is the licence to overwrite, not a courtesy a full store may skip: every image an operation will replace is kept first, *all together* (`artwork_store.keep_all`), and a target whose image was not retained is left as it is. Kept one at a time, the size cap's sweep for the second image could evict the first, whose caller already held a digest and had gone on to overwrite the original. The refusal narrows the artwork, never the rest of the work — a re-tag still writes its tags, additions still happen — and it is reported once, attributed to the album; the store's own log lines stay out of the feed. A store configured with a zero cap therefore disables replacement, not merely its undo.

**Bounded, and honest about it.** The promise is per album — its last `keep_per_album` (five) artwork changes can be undone, however busy the rest of the library is (#408) — with a byte cap (default 500 MB) underneath as a backstop that evicts oldest-first. The backstop can still make an old enough undo unavailable. The album page checks availability *before* rendering, so no Undo button is offered for a change whose image has gone — a button that would fail is worse than none. Usage is shown in Settings, because "how much disk is this costing me" is the question the cap exists to answer.

**Restore resolves everything before writing anything.** A partial restore would leave the album in a state that never existed and neither half revertable, so `restore_artwork` reads every image first and raises `ArtworkUnavailableError` if any is missing. It keeps what it overwrites — an undo is itself a destructive write — and refuses the whole undo if any of that cannot be kept. It skips files already correct, so running it twice is a no-op.

**An addition is undone back to absence** (#471). A record `[None, digest]` — a track that had no image, a folder cover a change created — is in the plan (`tag_history.artwork_revert_plan`) like a replacement, and its undo takes the image off (`formats.remove_cover`) or deletes the created `cover.*`. It needs nothing from the store, so it stays undoable however full the store gets. **Every target is checked against the `after` the change recorded**: a file carrying something else has been changed since, and is left alone and named, the way a tag Undo leaves a field. Older folder-cover creations that recorded only a `cover.write` line carry no before/after, and are not offered — there is no evidence to undo them with. It writes through `formats.write_cover`, deliberately separate from `write_tags`: undoing an artwork change must not silently rewrite tags as well.
...
**A record names a file relative to the album's root** — the deepest directory every one of its files is under (`album_files.Naming`). For an album in one folder that is the bare filename records have always carried; for an album whose discs sit in sibling folders it is `CD2/01 - Intro.m4a`, which the primary directory alone could not express. Undo resolves a name by looking it up among the album's own files rather than joining it onto a directory, so a record can never address a file outside the album, and a name from before this scheme still finds its file by matching the tail of its path. A name two files answer to — `01.m4a` on both discs, from a record written before #423 — is **refused**: there is nothing to choose between them, and an undo that quietly puts one disc's tags on the other's file is a lie about the user's tags that reports success. Both `revert_tags` and `restore_artwork` therefore take the album's folders, not just its primary directory.
...
**Tag undo follows renamed files** (#167). If the recorded filename is absent, resolve by release-track ID, then recording ID plus disc/track position, then position alone, always within the album. A lookup with no match permits the next; ambiguity refuses the undo rather than weakening the evidence. Two records may never claim the same file. All lookups use pre-write tag snapshots, and the release-identity check uses those same resolved paths, so renaming does not prevent a complete undo from restoring the sidecar's release link. The undo's own history records the resulting IDs and position, making a later rename resolvable in turn. Artwork undo still uses its recorded filenames.

**The undo records itself** as `tag.revert` with its own per-field detail, so it appears in History and is undoable in turn.

**`mb_album_id` is all-or-nothing, and the sidecar follows it** (#158). Identity is the one field that isn't per-file: the sidecar records exactly one release, so a revert that moved the id on some files and left it on others would derive as `INCONSISTENT` and leave nothing coherent to write down. So `_identity_revert` decides it once for the album — every file in the plan must agree on the before-value, still carry the after-value, and account for every file the album has *in all of its directories* — or the field is left alone and reported stale.
...
**Release review names the artwork outcome** (#593). The optional inclusion checkbox starts off; checking it names the embedded images and/or folder cover in the existing plan's changes beneath the preview. With no changes, the review states that artwork is kept and distinguishes protected per-track images from a selected image already present locally. Only matching image digests collapse duplicate previews; dimensions and rounded sizes cannot establish identity. Refresh preserves the inclusion choice and its displayed outcome. This changes the presentation, not the plan or the single confirmation action.

**Artwork inspection preserves scale** (#594). Comparison thumbnails keep equal frames and contain the source aspect ratio even beside long labels. Album and assignment-review thumbnails open native popovers with a shared fit scale and a 100% mode (one source pixel per CSS pixel); only the image area scrolls, leaving dimensions and controls visible. Switching distinct images within that section carries the scale mode and scroll position across. The viewer uses the existing digest-addressed image routes, including the selected replacement release, and keeps its inspection state only in the open page.

The review aligns its candidate with the first current-image row the plan would change, sharing one heading row even when labels wrap. Its artwork comparison replaces the separate album Artwork section while a suggestion is open; cancelling or completing the review restores that section.
...
- On selection, POST to `/manual/{id}/assign`. The assign tags the album and
  ...
  download reaches Needs Linking rather than Complete.
...
- **Live count updating during the sync phase** *(nice to have)* — the
  reconcile pass already publishes live inbox/library/New/Needs Linking
  ...
  NEEDS_SYNC → COMPLETE, decrement Needs Linking / increment Library live —
