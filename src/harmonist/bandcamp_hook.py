...
class HarmonistSyncer(_BCSyncer):  # type: ignore[misc]
    ...
    def __init__(
        ...
        *,
        ...
        # Set while any album is still Needs Linking, so we never re-download a copy
        # of an album already on disk; downloads resume next sync (Needs Linking 0).
    ...
    def _adopt_link(self, item: Any, url: str) -> bool:
        ...
        with activity_store.action():
            if write_sidecar_for_item(item, album_dir, prefer_item_url=True):
                ...
                activity.record(
                    f"Needs Linking → Library (auto-linked to Bandcamp purchase "
    ...
    def _link(self, album_dir: Path, item: Any) -> None:
        try:
            ...
            with activity_store.action():
                if write_sidecar_for_item(item, album_dir, prefer_item_url=True):
                    # A linked album leaves Needs Linking for the Library. Record the
                    # transition in the Activity feed (and server log).
                    activity.record(
                        f"Needs Linking → Library (linked to Bandcamp purchase "
    ...
    def sync_item(self, item: Any, encoding: str | None = None) -> bool:
        ...
        if url:
            ...
            if existing_dir is not None:
                ...
                # "Needs Linking → Library" for albums long since in the Library.
                #
                ...
                if not already_linked and not unreadable:
                    try:
                        ...
                        activity.record(
                            f"{getattr(item, 'band_name', '?')} — "
                            f"{getattr(item, 'item_title', '?')}: Needs Linking → Library "
        ...
        # subdomain + title). If it links, the album leaves Needs Linking for the
